...
class InstaBot:
    ...
    def __init__(self,
        ...
        self.location_id_list = location_id_list
        # Get random tag, from tag_list, and like (1 to n) times.
        self.max_like_for_one_source = max_like_for_one_source
        ...
        if (self.login_status):
            ...
            if self.login_status == 1:
                ...
                try:
                    ...
                    self.media_found = list(all_data['tag']['media']['nodes'])
                except:
                    self.media_found = []
    ...
    def get_media_id_by_location(self, location_id):
        """ Get media ID set, by your location id """

        if (self.login_status):
            log_string = "Get media id by LOCATION: %s" % (location_id)
            self.write_log(log_string)
            if self.login_status == 1:
                url_location = self.url_location % (location_id)
                try:
                    ...
                    self.media_found = []
                    self.write_log("Except on get_media_by_location(): %s" % str(e))
            else:
                return 0
    ...
    def like_all_exist_media(self, media_size=-1, delay=True):
        ...
        if self.login_status:
            if self.media_found != 0:
                i = 0
                for d in self.media_found:
                    # Media count by this tag.
                    if media_size > 0 or media_size < 0:
                        media_size -= 1
                        l_c = self.media_found[i]['likes']['count']
                        if ((l_c <= self.media_max_like and
                            ...
                            log_string = "Trying to like media: %s" % \
                                         (self.media_found[i]['id'])
                            self.write_log(log_string)
                            like = self.like(self.media_found[i]['id'])
                            # comment = self.comment(self.media_found[i]['id'], 'Cool!')
                            # follow = self.follow(self.media_found[i]["owner"]["id"])
                            if like != 0:
    ...
    def new_auto_mod(self):
        while True:
            if len(self.media_found) == 0:
                if not (self.location_id_list and self.tag_list):
                    self.write_log('No tags and location IDs were given!')
                    sys.exit(1)
                if self.tag_list and not self.location_id_list:
                    self.get_media_id_by_tag(random.choice(self.tag_list))

                    self.this_source_like_count = 0
                    self.max_source_like_count = random.randint(
                        1, self.max_like_for_one_source)


                elif self.location_id_list and not self.tag_list:
                    self.get_media_id_by_location(random.choice(self.location_id_list))

                    self.this_source_like_count = 0
                    self.max_source_like_count = random.randint(
                        1, self.max_like_for_one_source)

                # if you want to like both by tags and locations
                else:
                    # choose from which source you will get the media
                    tag_function = lambda : self.get_media_id_by_tag(random.choice(self.tag_list))
                    location_function = lambda : self.get_media_id_by_location(random.choice(self.location_id_list))
                    random_function = random.choice([tag_function, location_function])
                    random_function() #run it!
                    self.this_source_like_count = 0
                    self.max_source_like_count = random.randint(
                        1, self.max_like_for_one_source)
            # ------------------- Like -------------------
    ...
    def new_auto_mod_like(self):
        if time.time() > self.next_iteration["Like"] and self.like_per_day != 0 \
            ...
            if self.like_all_exist_media(media_size=1, delay=False):
                ...
                self.this_source_like_count += 1
                if self.this_source_like_count >= self.max_source_like_count:
                    self.media_found = [0]
            # Del first media_id
    ...
    def new_auto_mod_follow(self):
        if time.time() > self.next_iteration["Follow"] and \
                        self.follow_per_day != 0 and len(self.media_found) > 0:
            if self.media_found[0]["owner"]["id"] == self.user_id:
                self.write_log("Keep calm - It's your own profile ;)")
                return
            log_string = "Trying to follow: %s" % (
                self.media_found[0]["owner"]["id"])
            self.write_log(log_string)

            if self.follow(self.media_found[0]["owner"]["id"]) != False:
                self.bot_follow_list.append(
                    [self.media_found[0]["owner"]["id"], time.time()])
